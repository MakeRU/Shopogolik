...
<form action="newordus.php" method="post">
	<table>
		<tr>
...
//	$ordsru = (isset($_POST['Sum_RU'])) ? mysql_real_escape_string($_POST['Sum_RU']) : '';
	...
	if ($ordshop == 7) 
	{
	$ordsus = $ordcost * ((100-$orddisc)/100) *((100+ $ordtax) /100)*((100+ $ordorg) /100) + $ordship;
	
	if ($ordcost <= 5) 
		{	
			$ordsus = $ordcost * ((100-$orddisc)/100) *((100+ $ordtax) /100) + 1 + $ordship;
			$ordorg = 1;
		}
	}
	...
	// проверяем на наличие ошибок (например, длина логина и пароля)
		...
		print '<h4>Заказ добавлен</h4><a href="allord.php">Все заказы</a>';
		header('Location: newordus.php');
	}
}
include "footer.php"; 
?>